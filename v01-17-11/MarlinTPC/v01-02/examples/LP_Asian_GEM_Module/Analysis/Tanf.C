...
#ifdef __MODULE3__
void Tanf(Int_t module = 3, Int_t row = 16)
#endif
#ifdef __MODULE0__
void Tanf(Int_t module = 0, Int_t row = 23)
#endif
{
  //const Int_t kNp = 11;
  //const Int_t kNp = 9;
  ...
  titlestr << "tan#phi (Module" << module << " Row" << row << ")" << ends;
  ...
  grp->GetHistogram()->GetYaxis()->SetTitle("tanf");
...
void Process(Int_t run, Int_t layer, Double_t &x, Double_t &y, Double_t &dx, Double_t &dy)
{
  ...
  const Double_t kTanfMinCut = -0.015;
  ...
  hits->Draw("tanf", cut.str().data());
  TH1D* htemp = static_cast<TH1D* >(gROOT->FindObject("htemp"));
  htemp->Rebin(3);
  //htemp->Fit("gaus","","",-0.01,0.01);
  htemp->Fit("gaus","","",kTanfMinCut,kTanfMaxCut);
